...
ORDER BY s.custom_year;
    ...
    plt.plot( years[:], savings_norm01, label="μέσα ημερήσια αποθέματα νερού", color="deepskyblue" )
    plt.plot( years[:], precipitation_norm01, label="συνολικός ετήσιος υετός", color="lightskyblue", linestyle='dashed' )
    plt.legend() 
    plt.grid()
    plt.show()
...
def run_pearson():
    ...
    plt.figure( figsize=( 10, 6 ) )
    plt.suptitle( 'Μεταβολή στα αποθέματα νερού και τις μετρήσεις υετού', fontsize=14 )
    plt.xlabel( 'Έτη', fontsize=12 )
    plt.ylabel("Μεταβολή ως προς την προηγούμενη τιμή", labelpad=12)
    plt.tick_params( axis='x', labelrotation=90, labelsize=9 )
    plt.tick_params( axis='y', labelrotation=0, labelsize=9 )
    plt.plot( years[:], savings_change, label="Μέση ημερήσια ποσότητα αποθεμάτων", color="deepskyblue" )
    plt.plot( years[:], precipitation_change, label="Σύνολο ετήσιων μετρήσεων υετού", color="lightskyblue", linestyle='dashed' )
    plt.legend() 
    plt.grid()
    # plt.show()
    ...
    plt.figure( figsize=( 10, 6 ) )
    plt.suptitle( 'Κανονικοποίηση τιμών στα αποθέματα νερού και τις μετρήσεις υετού', fontsize=14 )
    plt.xlabel( 'Έτη', fontsize=12 )
    plt.ylabel("Κανονικοποίηση από 0 έως 1", labelpad=12)
    plt.tick_params( axis='x', labelrotation=90, labelsize=9 )
    plt.tick_params( axis='y', labelrotation=0, labelsize=9 )
    plt.plot( years[:], savings_norm01, label="Μέση ημερήσια ποσότητα αποθεμάτων", color="deepskyblue" )
    plt.plot( years[:], precipitation_norm01, label="Ετήσιο σύνολο μετρήσεων υετού", color="lightskyblue", linestyle='dashed' )
    plt.legend() 
    plt.grid()
    plt.show()
